...
Write a function named templateWithJQuery that uses jQuery to get the html template from the DOM,
 copy the contents,
...
const templateWithJQuery = () => {
  starWarsPeople.forEach(x => {
    const item = $('#template').html();
    var template = $('<section>' + item + '</section');
    template.find('h2').text(x.name);
    template.find('h3').text(x.height);
    template.find('p').text(x.eye_color);
